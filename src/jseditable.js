...
*/
function getTextNodes(nodeList, resolveMentions) {
...
*/
function getText(event, textArea, callback) {
    /**
...
/**
 * From a mention node get the mention display name
*/
function getDisplayName(node) {
    ...
    var nodes = getTextNodes(node),
    text, name = '';

    for (var i = 0; i < nodes.length; ++i) {
        try {
            text = normalizeSpace(nodes[i].nodeValue);
            // not perfect, but good enough. In some cases
            // there will be blank HTMLElements like the
            // <span> that contain unnecessary whitespace
            if (/^\s+$/.test(text))
                continue;
            name += text;
        } catch (e) { continue; } // HACK ignore fucked up nodes
    }
...
*/
function getSelectedTextNodeOffset(node) {
    var range, newOffset;
...
*/
function selectNodeText(node, start, end) {
    var sel, range;
...
/**
 * Remove the seach term from the given text node
*/
function selectSearchTerm(user, debug) {
    if (debug) return;
    var comparatorType = comparatorTypeMap[user.identifier],
    name = user[comparatorType].toLowerCase(),
    endOffset = offset,
    i = 0,
    text = normalizeSpace(anchor.nodeValue.slice(offset)).toLowerCase();

    while (i < text.length && text[i] == name[i]) {
        ++endOffset;
        ++i;
    }

    selectNodeText(anchor, offset - 1, endOffset);
}
